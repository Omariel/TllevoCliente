...
import 'package:tlleva/Pages/Login/welcome.dart';
...
class Api {
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) => Phone(
                    name: '', email: email, password: password, login: true)));
    ...
    if (response.statusCode == 200) {
      ...
      if (payload['message'] == 'Error, email no válido') {
        ...
        if (isForgot == true) {
          // ignore: use_build_context_synchronously
          Navigator.push(context,
              MaterialPageRoute(builder: (context) => RecuperarPassword(id: payload['user_id'].toString(),)));
        } else {
  ...
  Future<void> resetPassword(
    ...
    if (response.statusCode == 200) {
      ...
      if (success == false) {
        // ignore: use_build_context_synchronously
        Const().snackBarErrorSuccefull(context, 'Ha ocurrido un error, vuelve a intentarlo más tarde', Colors.red);
      } else if (success == true) {
        // ignore: use_build_context_synchronously
        Const().snackBarErrorSuccefull(context, 'Se ha cambiado exitosamente!', Colors.green);
  ...
  //message == Debe iniciar sesión - success == false
  //"message": "success logout" - success == true
  Future<void> logOut(
      String tokenUser, BuildContext context) async {
    var response = await http.get(
